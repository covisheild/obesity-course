...
- **`sources/INDEX.yml` says which file backs which citekey**, and it is not guessable from the filename. `constitution` resolves to `constitution_current.txt`; `constitution.txt` is an older consolidation (newest amendment it names: 2006) that does not contain Article 279A at all and is marked do-not-cite. Citing a do-not-cite file blocks. It is for the auditor; the reader meets it inside the reference entry, never in the sentence.
