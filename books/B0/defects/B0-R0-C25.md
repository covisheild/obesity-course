...
**M-1** · `exercises[1].answer` ¶3 · **style** · found at the merge build (check_report.md): the
round-2 sentence "Have them check the household case at the ceiling itself: if coverage is 0.75, …"
ran to 26 words. The round-2 verifier read the build's summary line, which does not list warnings.
**Main thread:** split into three sentences, words unchanged.
**Verifier:** closed
