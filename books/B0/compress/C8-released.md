...
inside an interval, the rectangle over-counts or under-counts. Take the largest gap between the
...
the quantity's unit. Narrower intervals shrink it without removing it.
...
Cut the intervals narrower and the error shrinks. It does not vanish.

The total under a rate line is called an integral. Nothing in this book asks you to work one out.
...
Narrower intervals shrink that error and never remove it.

And every amount here is an amount going one way. A flow out of something is an amount subtracted
rather than added, and the running total then falls.
...
- A total carries its interval assumption with it, and narrower intervals shrink the error without ever removing it.
