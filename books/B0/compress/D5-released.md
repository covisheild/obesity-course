...
**In plain terms.** Take six numbers: 5, 8, 12, 15, 20, 40. Add them and divide by how many there are for the mean.
...
Both rows fail the same way, and each one passes against the other row's numbers. The numbers
fit if the two rows' quartiles were printed in each other's places.
...
stretch running upward than downward. That is exactly the shape the section on variation
described in words.
