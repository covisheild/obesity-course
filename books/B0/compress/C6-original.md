...
**Definition.** Three shapes cover most of the relationships this course will draw, and they are told apart by
what equal steps in the input do to the output.

A relationship is linear when equal steps in the input add the same amount to the output every
time. Its graph is a straight line, and its slope is that amount divided by the size of the
step.

A relationship is exponential when equal steps in the input multiply the output by the same
factor every time. Its increments are proportional to the output itself.

A relationship is saturating when the output rises while its increments shrink towards zero,
approaching a value it does not pass. That value is the ceiling, and the output may approach it
...
difference by the difference before it. A ratio that holds at some number below one means each
difference is a fixed share of the one before, so the differences sum to a finite amount and a
ceiling exists. Where that ratio does not hold below one, shrinking differences establish no
ceiling, and the shape has not been named.

An exponential relationship drawn with a logarithmic vertical axis is a straight line, because
taking log10 of the output turns a constant ratio into a constant difference.

**In plain terms.** You can read a graph now. This section is about naming what you are looking at, which is a
separate skill and comes first.

Three shapes turn up over and over. You can tell them apart by eye, and then check by hand.
...
starts off looking lazy and then leaves everything else behind.
...
Now the check by hand, for when your eye is not enough. It needs values taken at equal steps
along the bottom, and it is a few subtractions and a few divisions.
...
than the step before it. The ruler goes on for ever. Nothing on it is a level to flatten
against.

So run one more division, and this time divide the differences rather than the values. Take each
difference and divide it by the difference before it. Does that ratio hold at some number below
one? Then each difference is a fixed share of the one before it, the differences add up to a
finite amount, and there is a ceiling. That is saturating.

Differences that shrink without holding a ratio may be heading nowhere in particular. That test
has then found you no ceiling, so do not go looking for one on the strength of the shrinking
alone.
...
points can be drawn as any of the three shapes here. You need enough of the run to see what the
steps are doing.
...
Test it before you agree. Divide each value by the one before it.
...
Three, then 1.4. The ratios are not the same, so nothing here is multiplying by a fixed factor.
Exponential is the wrong word. It was reached by looking at a rise rather than by testing one.

Now take the run out to six values, and put two other shapes beside it for comparison.
...
The ratio holds at about 0.6 all the way down the column, and 0.6 is below one. So each step
closes about the same share of whatever distance is left, the shrinking differences add up to
something finite, and a ceiling exists. Here it is 60. The column gets closer to it at every
step and does not arrive.
...
That is why you take the run out before you name anything. Here the exponential column is the
flattest at the start and the steepest at the end. A short run shows you the opposite of what
a long run shows.

Do not carry that away as a property of exponentials. It is a property of these three columns,
which somebody scaled against each other. Start all three at 10. Let the exponential multiply
by 3 at every step and the linear add 10. The exponential now leads from the first step.

What always holds is the other half. An exponential eventually passes any linear.

**Where this picture breaks.** These three shapes are a starting kit and not a list of everything. A curve that falls, one
that rises and then falls, and a line with a step in it are all things you will meet. None of
them is here.
...
And naming a shape is not explaining it. A saturating curve says that something is running out
of room. It does not say what.

**Must know points for you.**

- Run two tests on any set of values taken at equal steps. Subtract each from the next for a constant difference, and divide each by the one before for a constant ratio. A constant difference is linear and a constant ratio is exponential, and the pair of tests takes under a minute.
- Shrinking differences do not on their own mean a ceiling. A quantity can climb for ever by ever smaller amounts. A logarithm does exactly that. So run one more test before you say saturating. Divide each difference by the difference before it. A ratio that holds below one means the differences add up to a finite amount, so there is a ceiling to ask about. Where no such ratio holds, you have no ceiling to quote and should say so rather than estimate one.
- Exponential does not mean fast. It means each equal step multiplies by the same factor. An exponential rise can be the gentlest-looking of the three at the start, so a steep early rise is no evidence for the word.
- Three points have no shape. Any three rising values can be drawn as a line, a curve that takes off, or a curve that flattens. Ask for the rest of the run before you name anything.
- Read the side axis before you call a line linear. On an axis where each step multiplies by ten, a straight line is an exponential. The tamest-looking picture is then the one that is multiplying.
- Where a curve has flattened, a further equal step in the input buys much less than the step before it did. Say that out loud when somebody in a meeting proposes doubling the input. Then say what you would need in order to find where the flat part begins.
- Naming a shape names a pattern and never a mechanism. A saturating curve does not tell you what ran out, and a reader who takes the name as an explanation has stopped one question early.
- When a journalist uses the word exponential, ask what the values were at equal times and whether the ratio held. A rise called exponential in public may well not be one, and the honest sentence is that it grew and here is by how much.
...
**1.** Each of these is a set of values taken one step apart. Name the shape of each, and say which
test told you.
