...
very repeated measurement on a badly calibrated instrument is not a truer measurement. It is a more
confident wrong one.
...
kilograms. Two stated scales are each weighed against it twenty times.
...
- Measurement bias is the VIM's own word (entry 2.18) for the same shape of problem. It is an error that points one way and does not shrink when you collect more.
