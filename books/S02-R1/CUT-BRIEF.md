# Cutter brief · S02-R1 (Task 5, step 5a)

Repository `/home/claude/obesity-course`. Do not commit, push, or edit any record or any file not
named here. Read `claude.md` §11, §11a and §12. Read `books/S02-R1/compress/<SECTION>-original.md`
(the section as the reader meets it) and `books/S02-R1/compress/<SECTION>-prose.yml` (the prose
fields, which are all you may touch).

Write a deliberately aggressive compression of that section to
`books/S02-R1/compress/<SECTION>-pass1-prose.yml` (same fields, same YAML shape as `-prose.yml`).
...
Then run `python check/compress/validate.py --subject S02-R1 <SECTION>-pass1-prose.yml` until it
passes (deletion only, sentences verbatim, mean and longest sentence not risen — if it fails, fix
your cut, never the tool), and `python check/compress/prepare.py --subject S02-R1 assemble <SECTION>`.

Append to `books/S02-R1/compress/LEAST-SURE-<SECTION>.md` the one cut you were least sure about
