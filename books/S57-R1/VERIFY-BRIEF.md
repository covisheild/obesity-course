# Verifier brief · S57-R1 (Task 4b of PIPELINE.md)
...
For each defect in `books/S57-R1/defects/<RECORD-ID>.md`, read the fixer's "Fixer:" line, then
read the record (`check/records/S57/<RECORD-ID>.yml`) and check the defect is actually gone:
...
(`books/S57-R1/FIX-BRIEF.md`); check it. Check nothing else. Under each defect write one line:
